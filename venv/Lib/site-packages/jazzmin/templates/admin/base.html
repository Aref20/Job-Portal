        ...
        {% if jazzmin_settings.show_sidebar %}
            ...
            <aside class="main-sidebar elevation-4 {{ jazzmin_ui.sidebar_classes }}" id="jazzy-sidebar">
                <a href="{% url 'admin:index' %}" class="brand-link {{ jazzmin_ui.brand_classes }}" id="jazzy-logo">
                    <img src="{% static jazzmin_settings.site_logo %}" alt="{{ jazzmin_settings.site_header }} Logo" class="{{ jazzmin_settings.site_logo_classes }} brand-image elevation-3 mr-3" style="opacity: .8">
                    <span class="brand-text font-weight-light mr-2">{{ jazzmin_settings.site_brand }}</span>
                </a>
...
<script>
    for(i=1;i<=3;i++)
        document.querySelector("#jazzy-collapsible").appendChild(document.getElementsByClassName("card card-default")[1])
</script>
</body>
</html>
